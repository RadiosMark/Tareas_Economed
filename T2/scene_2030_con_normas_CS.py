...
# Leer el archivo 'datos_t2.xlsx', hoja 'existentes', rango 'AB7:X2695'
...
print("📝 Generando reporte de resultados para el escenario 'Política Eficiente'...")

# Nombre del archivo de salida
nombre_archivo = 'resultados_modelo_normas_CS.txt'

# --- INICIO DEL SCRIPT ---
try:
    # Cargar la solución para asegurar que los valores estén disponibles
...
if results.solver.termination_condition == pyo.TerminationCondition.optimal:
    ...
    # Cargar el parámetro de emisión de CO2 si aún no existe
    if not hasattr(model, 'factor_emision_CO2'):
    ...
    costo_social_total_escenario_eficiente = pyo.value(costo_social(model))

    # --- ESCRITURA DEL ARCHIVO .TXT ---
    
    with open(nombre_archivo, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("      REPORTE DE RESULTADOS: ESCENARIO 'POLÍTICA EFICIENTE' (CON NORMAS Y CS)\n")
        f.write("="*80 + "\n\n")

        # --- 4) COSTO TOTAL DEL SISTEMA ---
        costo_total = pyo.value(model.obj)
        f.write("--- 4. Costo Total del Sistema (incluye Costo Social) ---\n")
        f.write(f"Costo Óptimo Total: {costo_total / 1e6:.4f} MMUS$\n\n")
        
        # --- 5) BENEFICIO DE LA NORMA ---
        f.write("--- 5. Beneficio de la Norma ---\n")
        f.write(f"Costo Social de la Contaminación (Daño Ambiental) en este escenario: {costo_social_total_escenario_eficiente / 1e6:.4f} MMUS$\n")
        f.write("NOTA: El 'Beneficio' se calcula comparando este valor con el de un escenario base (ej. 'Sin Norma').\n")
        f.write("      Beneficio = (Costo Social SIN Norma) - (Costo Social CON Norma y CS).\n\n")
